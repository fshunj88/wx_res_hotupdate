...
namespace HotUpdate {
    ...
    export class MiniGameHotUpdateMgr {
        ...
        public constructor() {
            ...
            this.REMOTE_SERVER_ROOT = `${CUR_RES_URL_ROOT}`;
            // 读取本地版本号0.2.58
            ...
            if (cachedVersion) {
                ...
                if (this._cachedVerCompareToPackVerRet > 0) {
                    ...
                    this.REMOTE_SERVER_ROOT = `${HOT_UPDATE_URL_ROOT}`;
                }
            }
        }
        ...
        public async hotUpdate(): Promise<{ success: boolean, reason?: string }> {
            ...
            if (ret.isSuccess()) {
                ...
                if (remoteVerCompareToLocalVerRet > 0) {
                    // 有更新，下载json包,动态切换资源服务器为HOT_UPDATE_URL_ROOT
                    ...
                    if (this._cachedVerCompareToPackVerRet > 0) {
                        ...
                        this.REMOTE_SERVER_ROOT = `${HOT_UPDATE_URL_ROOT}`;
                        let re = await this._loadSettingsFromCached();
                        if (re) {
                            return { success: true };
                        } else {
                        ...
                        if (remoteVerCompareToLocalVerRet == 0) {
                            // 且跟最新版本号一样，应该删掉本地缓存吗？
                            // 不用删。此时settings已经从代码包里加载过了,直接用代码包的即可,远程服务器也用cur而不是latestVer
                        }
                        return { success: true };
                    }
                }
