...
declare const tt: {
     ...
     */
    getUserInfo(object: {success?:(res:any)=>void,fail?:(res:any)=>void,complete?:(res:any)=>void},withCredentials=false): void;
     ...
     * env 环境配置 0
     * currencyType 币种 CNY
     * platform 申请接入时的平台 android
     * buyQuantity 购买数量
     * zoneId 游戏服务器大区 id，游戏不分大区则默认填写"1"。如果应用支持多角色，则角色 ID 接在分区 ID 后，用"_"连接
     * customId 开发者自定义唯一订单号
     * extraInfo 开发者自定义其他信息
     */
    requestGamePayment(object: {mode:string,env:number,currencyType:string,platform:string,buyQuantity:number,zoneId?:string,customId?:string,extraInfo?:string,success:(res:any)=>void,fail:(res:any)=>void,complete:(res:any)=>void}): void;
    /**
     * 创建激励视频广告组件。请通过 wx.getSystemInfoSync() 返回对象的 SDKVersion 判断基础库版本号 >= 2.0.4 后再使用该 API。同时，开发者工具上暂不支持调试该 API，请直接在真机上进行调试。
     */
    createRewardedVideoAd(object: {adUnitId: string}): videoAd;
    /**
     * 头条小游戏暂不支持分包加载
     * 触发分包加载
     */
    ...
    /**
     * 创建录屏分享按钮。按钮在被用户点击后会发起对最近一次录制完成的视频的分享。
     * @param style
        按钮的样式
        属性	        类型	    默认值	    必填	说明
        left	        number		            是	左上角横坐标，单位 逻辑像素
        top	            number		            是	左上角纵坐标，单位 逻辑像素
        width	        number		            是	宽度，单位 逻辑像素
        height	        number		            是	高度，单位 逻辑像素
        icon	        string	默认图标，24x24	  否	图标的 url
        iconMarginRight	number	8	            录屏图标和文字之间的距离，单位 逻辑像素
        fontSize	    number	17	            否	字号
        color	        string	#ffffff	        否	字号
        paddingLeft	    number	16	            否	按钮的左内边距，单位 逻辑像素
        paddingRight	number	16	            否	按钮的右内边距，单位 逻辑像素
     */
    createGameRecorderShareButton(style: Object): GameRecorderShareButton;
}
...
declare interface GameRecorderShareButton {
     ...
     * 分享配置
        属性	       类型	    说明
        query	    number	打开小游戏的 query
        bgm	        string	视频的背景音乐
        timeRange	number	剪辑区间
     */
    share: any;
    style: any;
    show(): void;
    hide(): void;
    onTap(callback: (res: any) => void);
...
declare interface GameRecorderManager {
     ...
     * @param {Object} object 
     */
    start(obj: {duration:number}): Promise<any>;
    onStart(callback: (res)=>void): void;
    /**
     * 结束录制游戏画面。结束录制后可以发起分享。
     * 录屏器只会保存最近一次录制完成的视频，后一次的录制结果会覆盖前一次的。
     */
    stop(): Promise<any>;
    /**
     * @param res.videoPath
     */
    onStop(callback: (res)=>void): void;
    /**
     * 暂停录制游戏画面。如果要恢复继续录制游戏画面，调用 GameRecorder.resume()
     */
    pause(): void;
    /**
     * 恢复录制游戏画面。
     ...
     * 
     * @param callback
     *   回调函数。回调函数在被执行时会传入事件对象作为参数。
     *   事件对象
     *      timeUpdate
            属性	    类型	    说明
            currentTime	number	当前视频录制的秒数
...
declare interface FileSystemManager {
     ...
     * 删除目录
     */
    rmdir(object: {dirPath:Object,recursive:boolean,success:(res:any)=>void,fail:(res:any)=>void,complete:(res:any)=>void}): void;
    /**
     * 读取目录内文件列表
     */
